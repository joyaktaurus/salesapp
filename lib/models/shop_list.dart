...
class ShopList {
  ...
  ShopList({
    this.message,
    this.shop, required List<ShopReg> result, required count,
  });
  List<ShopReg>? result;
  int? count;

  factory ShopList.fromJson(Map<String, dynamic> json) => ShopList(
    result: List<ShopReg>.from(
        json["result"].map((x) => ShopReg.fromJson(x))),
    count: json["count"],
  );
  ...
  //   shop: List<ShopReg>.from(json["shop"].map((x) => ShopReg.fromJson(x))),
  // );
  //
  // Map<String, dynamic> toJson() => {
  //   "message": message,
...
class ShopReg {
  ...
  factory ShopReg.fromJson(Map<String, dynamic> json) => ShopReg(
